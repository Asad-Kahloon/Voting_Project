...
const UpdateProvince = () => {
    ...
    const handleSubmit = (e) => {
        ...
        e.preventDefault();
        axios.put('http://localhost:3001/province/province/' + id, {electionname, provincename})
        .then(res => {
            if(res.data.updated){
    ...
    <main className="main-container">
      ...
      <div className="login-container">
        ...
        <div className="form-group">
          ...
          <input value={provincename} type="text" placeholder="Enter Province Name" 
          onChange={(e) => setProvinceName(e.target.value)} />
        </div>
        <div className="form-group">
          <label htmlFor="election">Election</label>
          <select value={electionname} name="election" id="election" 
          onChange={(e) => setElectionName(e.target.value)}>
            <option value="">Select Election</option>
            {
                titles.map(title => {
                 return  <option key={title.id} value={title.title}>{title.title}</option>
                })
            }
          </select>
        </div>
